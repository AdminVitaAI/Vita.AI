...
app.config['SECRET_KEY'] = 'vita_ai_secret_key_2024'
...
app.config['SQLALCHEMY_DATABASE_URI'] = f"sqlite:///{os.path.join(os.path.dirname(__file__), 'database', 'vita_ai.db')}"
...
def serve(path):
    ...
    if path != "" and os.path.exists(os.path.join(static_folder_path, path)):
        ...
        if os.path.exists(index_path):
            ...
            return {"message": "Vita.ai API is running", "version": "1.0.0"}, 200

@app.route('/api/health')
def health_check():
    return {"status": "healthy", "service": "Vita.ai API"}, 200
